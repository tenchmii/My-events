...
function OutingCard({ outing }: { outing: Outing }) {
  return (
    <div className="p-4 rounded-lg text-white">
      <p>
        Sortie organisée par <span className="font-bold text-blue-400">{outing.organizer.pseudo}</span>
      </p>
      <p>{outing.participant_count} participant(s)</p>
      <a href={`/outings/${outing.id}`} className="text-blue-400 hover:underline mt-2 inline-block">
        Voir les détails
      </a>
    </div>
  );
...
export default async function EventDetailPage({ params }: EventDetailPageProps) {
  ...
  return (
    <>
      <div
        className="relative max-w-7xl mx-auto mt-5"
        ...
        <div className="backdrop-blur-xl bg-gradient-to-b from-black/45 to-[#1E1E1E] p-6 lg:p-10 text-white">
          <div className="flex flex-col lg:flex-row lg:items-start gap-8 lg:gap-12">
            <div className="flex-1 flex flex-col gap-4">
              <h1 className="text-4xl font-extrabold leading-tight">{event.title_fr}</h1>
              <span className="text-sm sm:text-base text-gray-400">
                By: <span className="font-semibold text-white">{event.originagenda_title}</span>
              </span>
              ...
              <div className="border-t border-gray-700 pt-6">
                ...
                {outings.length > 0 ? (
                  ...
                  <p className="text-gray-400">
                    Aucune sortie publique organisée pour cet événement pour le moment.
                  </p>
                )}
              </div>
            </div>
